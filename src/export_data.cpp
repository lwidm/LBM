...
int save_eigen_matrix(Eigen::ArrayXXd &array,
    ...
    if (std::filesystem::exists(output_filename)) {
        ...
        if (save_flag != FORCE) {
            char choice;
            std::cout
                << "The save state file \"" << output_filename
                << "\" already exists.\nDo you want to overwrite it? (y/n): ";
            std::cin >> choice;

            if (choice != 'y') {
                std::ostringstream oss;
                oss << "Couldn't save the state of the current simulation "
                       "step: The "
                       "file \""
...
int save_state(const std::string &sim_name,
    ...
    err = save_eigen_matrix(state.ux,
                            sim_dir + "/" + additional_string +
                                "ux_t=" + time_str + ".bin",
                            save_flag);
    if (err != 0) {
    ...
    err = save_eigen_matrix(state.uy,
                            sim_dir + "/" + additional_string +
                                "uy_t=" + time_str + ".bin",
                            save_flag);
    if (err != 0) {
